...
function styleMain(){
     if(!$("#styleForm").length) return false;        
             $(".lazy.hidden.opv1.opv1.opv3").removeClass('hidden');
            imgCtr = $(".vertthumbnail").not(".hidden").length;
...
paramsOg.forEach((value, key) => {
  if (!paramsKeys.includes(key) && (key.substring(0, 4) == 'utm_' || key.substring(0, 3) == 'mc_' || key == 'gclid' || key == 'gad_source' || key == 'msclkid' || key.substring(0, 4) == 'cto_')) {
...
function checkFromUrl(mode){
 ...
 if(query!==undefined && query !==""){
  ...
  var doLoadPage = false;
    ...
    if (pair[0] != 'PMSRCE' && pair[0].substring(0,4) != 'utm_' && pair[0] != 'externalId' && pair[0].substring(0,3) != 'mc_' && pair[0] != 'gclid' && pair[0] != 'gad_source' && pair[0] != 'msclkid' && pair[0].substring(0,4) != 'cto_') doLoadPage = true;
	}
                                            ...
                                            $.each(json, function(key, value) {
                                                    switch (true){
                                                        ...
                                                        case (key == "vasable" ):
                                                            ...
                                                            if(value=="Y") {
                                                                $("#vasbutton").removeClass("hidden");
                                                                setTimeout(function(){newVasModel();},100);
                                                            } else {
                                                                $("#vasbutton").addClass("hidden");
                                                            }
...
function ImageChange(mode){
	...
	if(mode=='q'){
		if($("#Qopv1UL li.selected").length==1)
			opv1 = $("#Qopv1UL li.selected").attr("id").substring(4).replace("-",".");
		if($("#Qopv2UL li.selected").length==1)
			opv2 = $("#Qopv2UL li.selected").attr("id").substring(4).replace("-",".");
		if($("#Qopv3UL li.selected").length==1)
			opv3 = $("#Qopv3UL li.selected").attr("id").substring(4).replace("-","."); 
		
	}

colorImage(opv1,opv2,opv3,mode);
...
//*-*-*-**-*-*-**-*-*-**-*-*-**-*-*-**-*-*-**-*-*-**-*-*-**-*-*-**-*-*-**-*-*-**
function classhasImages(clssnm){
	var hasimages = false;
	if($(clssnm.trim()).length <=0) return hasimages;
	
	$(clssnm).each(function(){
		
        let img = $(this).attr("data-original");
		if(img.indexOf('no_image')<0){
			hasimages = true;
			return;
        }
    });		
...
let prefix = "";
if(modeq=='q') prefix ='.quickv';
let firstcolor = $('.choices a').first().attr("data-class");
if(modeq=='q')firstcolor = $("#QuickViewForm .choices a").first().attr("data-class");
...
try{
	...
	let binary = padWithZeroes(convertDecimalToBinary(optColor),3);
		...
		if (clssnm!=="" && clssnm!==prefix && classhasImages(clssnm.trim())) {
			break;
		}else{
            clssnm = ""+prefix.trim();
        }
...
pvimgclass = clssnm;
	...
	if(modeq =='q')setPrevNextQ();
    if(modeq!=='q')setPrevNext(); //
}
//*-*-*-**-*-*-**-*-*-**-*-*-**-*-*-**-*-*-**-*-*-**-*-*-**-*-*-**-*-*-**-*-*-**
function setPrevNextQ(){ 
    var hght = $("#QuickViewForm #product__thumbnails--scroll").find('img').first().height(); 
        hght = parseInt(hght)+12;
    var posTop = - posCtr*hght;
               $("#QuickViewForm #hiddenOverflow").animate({"top":posTop+"px"});
            
    if(imgCtr<=5){ $("#QuickViewForm #product__thumbnails--next").fadeOut(); $("#QuickViewForm #product__thumbnails--prev").fadeOut();return true;} 
    if(posCtr == 0){ $("#QuickViewForm#product__thumbnails--prev").fadeOut(); $("#QuickViewForm #product__thumbnails--next").show();return true;} 
    if(posCtr == (imgCtr -5)){ $("#QuickViewForm #product__thumbnails--prev").show(); $("#QuickViewForm #product__thumbnails--next").fadeOut();return true;} 
    if(posCtr > 0){ $("#QuickViewForm #product__thumbnails--prev").show();$("#QuickViewForm #product__thumbnails--next").show(); } 
...
function addToCartItem() {
    ...
    $(".iv2s").each(function () {
        if ($(this).attr("valrequired") == 'true_') {
            if ($.trim($(this).val()) == "") {
                $(this).addClass("uc_error");
                errors = "Y";
            } else {
                $(this).removeClass("uc_error");
            }
        }
    });
		...
		type: 'get',
            dataType: 'html',
             success: function (html) {
                ...
                /* (DX-0248 STAGE ONLY) */
                $('#miniCart').modal('hide');
                $("#samHeaderCartTotal").trigger("click");
...
// Remove 'letter size' input display on PDP VAS Customizations for name strips
...
function newVasModel() {
    showWait('B');
    var edp = $("#SIEDP").val();
    var qty = $("#SIQTY").val();
    var cdno = $("#SICDNO").val();
    $.ajax({
        url: '/CGBCXSTYL?SIACTN=LOADI1NO&SIEDP='+edp+'&SIQTY='+qty+'&SICDNO='+cdno,
        type: "get",
        dataType: 'html',
        success: function(html){ 
            $('#newVasModel').html(html);
            $("#newVasModelDiv").removeClass("hidden").removeClass("uc_customized_option_disabled");
            $(".uc_pd_option .uc_pd_option_step_title").click();
            var options = "";
            if ($("#SIOPV1").val() != '') options +="<span>" + $("#origDesc1").html() + ": " + $("#SIOPV1").val() + "</span>";
            if ($("#SIOPV2").val() != '') options += "<span>" + $("#origDesc2").html() + ": " + $("#SIOPV2").val() + "</span>";
            if ($("#SIOPV3").val() != '') options += "<span>" + $("#origDesc3").html() + ": " + $("#SIOPV3").val() + "</span>";
            if (options != "") {
                $(".uc_pd_option_summary").html(options);
                $(".uc_pd_option_summary").slideDown(500);
            }
...
function ucSelectApp(obj) {
    showWait('B');
    var edp = $("#SIEDP").val();
    var qty = $("#SIQTY").val();
    var i1no = obj.name.substring(4);
    vedp = obj.value;
    var cdno = $("#SICDNO").val();
    $.ajax({
      url: '/CGBCXSTYL?SIACTN=LOADV2NO&SIEDP='+edp+'&SIQTY='+qty+'&SII1NO='+i1no+'&SIVEDP='+vedp+'&SICDNO='+cdno,
      type: "get",
      dataType: 'html',
      success: function(html) { 
        $('#v2no_'+i1no).html(html);
        julieRemoveLetterSizeInput();
        julieSetLetterSizeOption();
        iframeResize();
        hideWait();
