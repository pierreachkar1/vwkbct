{
  ...
  function generateProdCardSlider(arrProducts, sectionName) {
    ...
    arrProducts.forEach(product => {
      const {styleID, title, imageURL, brand, ratingURL} = product;
  
        $(`#${sectionName} .carousel`).append(`
            <div class="b5-active id=${styleID}">
                <div class="card justify-content-between">
                    ...
                    <div>
                        <button class="product__customize btn btn-primary fs-6 w-100" id="btn_${styleID}" onclick="toggleCategoryDiv('${styleID}')">Customize</button>
                    </div>
                </div>
            </div>
        `);
  ...
  function generateProductDropdown(productList, sectionID) {
    productList.forEach(product => {
      const {styleID, title, imageURL, price, colorURL} = product;
  
      generateProductContainerDiv(styleID, sectionID);
  
      generateMainImageAndDetails(styleID, imageURL);
      generateProd_FeatureAndSpecs(styleID);
      generateProdDetailsHeader(styleID, title, price);
      generateProdDetailsOptions(styleID, colorURL);
      generateProdDetailsFooter(styleID);
  
      if (styleID.includes('coat') || styleID.includes('hat')) {
        generateProdDetailsCustomize(styleID);
      }
      
      // Initialize click behaviors for product option elements
      initializeOptionClickHandlers(styleID);
      initializeOptionToggle(styleID);
  
      // Initialize expand select option
      initializeExpandSelectOption(styleID);
    });
  }
  
  function generateProductContainerDiv(styleID, sectionID) {
  ...
  /* Generate Main Image */
  function generateMainImageAndDetails(styleID, imageURL) {
    $(`#${styleID} .uc_main_img`).html(`
  ...
  function generateProdDetailsOptions(styleID, colorURL) {
  ...
  function initializeOptionClickHandlers(styleID) {
    //Option 1 - Tracking Click Event
    $(`#${styleID} #opv1UL a`).click(function() {
      if ($(this).children("li").hasClass("selected")) {
          $(this).children("li").removeClass("selected");
          $(`#${styleID} #samUC_Option1_Selected`).html("Select an Option");
          // $(`#${styleID} #samUC_Option1_Selected`).css("color","#939393");
      } else {
          $(`#${styleID} #opv1UL a li`).removeClass("selected");
          $(this).children("li").addClass("selected");
          $(`#${styleID} #samUC_Option1_Selected`).html($(this).attr("title"));
          // $(`#${styleID} #samUC_Option1_Selected`).css("color","green");
      }
      allOptionsSelected(styleID);
    });
  
    //Option 2 - Tracking Click Event
    $(`#${styleID} #opv2UL a`).click(function() {
        if ($(this).children("li").hasClass("selected")) {
            $(this).children("li").removeClass("selected");
            $(`#${styleID} #samUC_Option2_Selected`).html("Select an Option");
            // $(`#${styleID} #samUC_Option2_Selected`).css("color","#939393");
        } else {
            $(`#${styleID} #opv2UL a li`).removeClass("selected");
            $(this).children("li").addClass("selected");
            $(`#${styleID} #samUC_Option2_Selected`).html($(this).attr("title"));
            // $(`#${styleID} #samUC_Option2_Selected`).css("color","green");
        }
        allOptionsSelected(styleID);
    });
  
    //Option 3 - Tracking Click Event
    $(`#${styleID} #opv3UL a`).click(function() {
        if ($(this).children("li").hasClass("selected")) {
            $(this).children("li").removeClass("selected");
            $(`#${styleID} #samUC_Option3_Selected`).html("Select an Option");
            // $(`#${styleID} #samUC_Option3_Selected`).css("color","#939393");
        } else {
            $(`#${styleID} #opv3UL a li`).removeClass("selected");
            $(this).children("li").addClass("selected");
            $(`#${styleID} #samUC_Option3_Selected`).html($(this).attr("title"));
            // $(`#${styleID} #samUC_Option3_Selected`).css("color","green");
        }
        allOptionsSelected(styleID);
    });
  }
  
  function initializeExpandSelectOption(styleID) {
    $(`${styleID} select.uc_sel_skillbadge`).change(function() {
      ...
      positions.forEach(pos => {
          if (showPositions.includes(pos)) {
              $(`${styleID} select.uc_sel_skillbadge_position_${pos}`).show(500);
          } else {
              $(`${styleID} select.uc_sel_skillbadge_position_${pos}`).hide(500);
          }
      });
  });
  }
  
  function initializeOptionToggle(styleID) {
      optionToggle(styleID);
  }
  ...
  async function getProducts(keyword, categoryPage, productNumber, productSort) {
    ...
    //coat SH4690
    //"CGBCXSUGG?SIWORD=SH4690&SIMODE=F"
  
    //ORIGINAL: `/CGBCSRCH?${params.toString()}`
    // '/cgi/CGBCXSSUB?SISTYL=SH4690'
    try {
      const response = await fetch('/cgi/CGBCXSSUB?SISTYL=SH4690', {
        method: 'GET',
        headers: {
  ...
  async function getOptions(style) {
    const params = new URLSearchParams({
      SISTYL: style
    });
  
    try {
      const response = await fetch(`/cgi/CGBCSTYL?${params.toString()}`, {
        method: 'GET',
        headers: {
          'Content-Type': 'application/x-www-form-urlencoded'
        }
      ...
      return {
        ...
        speculations: ['Bullet point 1', 'Bullet point 2']
      }
