...
//To hold hidden data (NOTE: Need to a div on GQ: samHPHiddenData)
let samProductId = 0;
let samProductStyle = "";
let samProductPrice = "";
...
for (i = 0; i < arrFeaturedProducts.length; i++) {
    ...
    '        <span id="samHP_'+ i +'">' + arrFeaturedProducts[i][4] + '</span>'+
    '    </div>'+
    '    <div class="hp--product--color">' + arrFeaturedProducts[i][5] + '</div>'+
    '</div>';

    
    /* Show PRICE automatically if SUCCESS */
    samProductId = i;
    samProductStyle = arrFeaturedProducts[i][0];
    samProductPrice = arrFeaturedProducts[i][4];

    $.ajax({
        url: "/CGBCXSUGG?SIWORD=" + samProductStyle,
        success: function(result){ 
            console.log("SAM: " + result);  
            document.getElementById('samHPHiddenData').innerHTML = result;
            if (document.getElementsByClassName('product__price').length > 0) {
                if (document.getElementsByClassName('product__price')[0].getElementsByTagName('span').length > 0) {
                    samProductPrice = document.getElementsByClassName('product__price')[0].getElementsByTagName('span')[document.getElementsByClassName('product__price')[0].getElementsByTagName('span').length-1].innerText;
                    document.getElementById('samHP_'+ samProductId).innerHTML = samProductPrice;
                }
